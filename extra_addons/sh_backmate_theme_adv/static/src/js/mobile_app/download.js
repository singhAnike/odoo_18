...
function _download(data, filename, mimetype) {
    ...
    // SOFTHEALER CUSTOM CODE HERE WE JUST ALWAYS FALSE IF 
    // CONDITION
    // if (self.URL) {
    if (false) {
 ...
 */
function configureBlobDownloadXHR(xhr, { onSuccess = () => {}, onFailure = () => {} } = {}) {
    ...
    xhr.onload = () => {
        ...
        const filename = header ? parse(header).parameters.filename : "Test";
...
/***
 * SOFTHEALER REPLACE BELOW STANDARD METHOD
 * BECAUSE SOFTHEALER WANT TO CALL configureBlobDownloadXHR METHOD
 * AND THAN _download method defined above
 */
