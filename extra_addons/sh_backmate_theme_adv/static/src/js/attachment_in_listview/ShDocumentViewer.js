...
export class ShDocumentViewer extends Widget {
    static template = "ShDocumentViewer";
    /**
     ...
     */
     start () {
        ...
        document.querySelectorAll('.o_viewer_img').forEach(img => {
            img.addEventListener('load', this._onImageLoaded.bind(this));
        });

        document.querySelectorAll('[data-toggle="tooltip"]').forEach(element => {
            new bootstrap.Tooltip(element, { delay: 0 });
        });
     ...
     */
    _updateContent (e) {
        var newRender = renderToElement('ShDocumentViewer.Content', {
            widget: this,
        });
        /*$('[data-toggle="tooltip"]').tooltip({delay: 0});*/
        document.querySelector('.o_viewer_content').innerHTML = '';
        document.querySelector('.o_viewer_content').append(newRender)
        this._reset();
    }
    /**
    ...
    _rotate (angle) {
        ...
        document.querySelectorAll('.o_viewer_img').forEach(img => {
            img.style.transform = this._getTransform(this.scale, new_angle);
            img.style.maxWidth = new_angle % 180 !== 0 ? `${document.documentElement.clientHeight}px` : '100%';
            img.style.maxHeight = new_angle % 180 !== 0 ? `${document.documentElement.clientWidth}px` : '100%';
        });
    ...
    _zoom (scale) {
        ...
        if (scale > 0.5) {
                document.querySelectorAll('.o_viewer_img').forEach(img => {
                    img.style.transform = this._getTransform(scale, this.angle || 0);
                });
                this.scale = scale;
            }

            document.querySelectorAll('.o_zoom_reset, .o_zoom_out').forEach(button => {
                button.classList.toggle('disabled', scale === 1);
            });
    ...
    _onDrag (e) {
        e.preventDefault();
        if (this.enableDrag) {
            const image = document.querySelector('.o_viewer_img');
            const zoomer = document.querySelector('.o_viewer_zoomer');

            const top = image.offsetHeight * this.scale > zoomer.clientHeight ? e.clientY - this.dragStartY : 0;
            const left = image.offsetWidth * this.scale > zoomer.clientWidth ? e.clientX - this.dragStartX : 0;
    ...
    _onEndDrag (e) {
        ...
        if (this.enableDrag) {
            ...
            document.querySelector('.o_viewer_img').style.cursor = '';
    ...
    _onImageLoaded () {
        /*$('.o_loading_img').hide();*/
        document.querySelector('.o_loading_img').style.display = 'none';
    }
    /**
    ...
    _onPrint (e) {
        ...
        var src = document.querySelector('.o_viewer_img').getAttribute('src');
    ...
    _onZoomReset (e) {
        ...
        document.querySelector('.o_viewer_zoomer').style.transform = '';
        this._zoom(1);
    }
    /**
     * @param {Event} ev
